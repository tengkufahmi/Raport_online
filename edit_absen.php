      ...
      <li class="nav-item">
        ...
        <div id="content-wrapper">
          ...
          <div class="container-fluid">
            ...
            include 'koneksi.php';
            $id = $_GET['no'];
            $query = mysqli_query($koneksi, "SELECT * FROM absensi join siswa on siswa.no_induk=absensi.no_induk where id_absen = '$id'");
            while($data = mysqli_fetch_array($query)){
              ?>
              <form method="POST" action="update_absen" enctype="multipart/form-data" class="col-md-12">
                <div class="row">
                  <div class="col-md-3">                
                    <input type="hidden" value="<?php echo"$data[ID_ABSEN]";?>" name="id"/>
                    <div class="form-group">
                      <label for="">Nama Santri : </label> <?php echo"$data[NAMA_SISWA]"; ?>                    
                    </div>
                                                        
                    <div class="form-group">
                      <label for="">Subuh</label>                            
                      <select name="subuh" class="form-control">
                       <option value="Hadir"<?php if ($data["SUBUH"] == 'Hadir' ) echo "selected" ; ?>>Hadir</option>
                       <option value="Izin"<?php if ($data["SUBUH"] == 'Izin' ) echo "selected" ; ?>>Izin</option>
                       <option value="Sakit" <?php if ($data["SUBUH"] == 'Sakit' ) echo "selected" ; ?>>Sakit</option>
                       <option value="Alfa"<?php if ($data["SUBUH"] == 'Alfa' ) echo "selected" ; ?>>Alfa</option>
                     </select>
                   </div>
                 </div>  
                 ...
                 <div class="col-md-3">                 
                  <div class="form-group mt-5"> 
                    ...
                    <select name="maghrib" class="form-control">
                     <option value="Hadir"<?php if ($data["MAGHRIB"] == 'Hadir' ) echo "selected" ; ?>>Hadir</option>
                     <option value="Izin"<?php if ($data["MAGHRIB"] == 'Izin' ) echo "selected" ; ?>>Izin</option>
                     <option value="Sakit" <?php if ($data["MAGHRIB"] == 'Sakit' ) echo "selected" ; ?>>Sakit</option>
                     <option value="Alfa"<?php if ($data["MAGHRIB"] == 'Alfa' ) echo "selected" ; ?>>Alfa</option>
                   </select>
                 </div> 
                 </div>                         
                 <div class="col-md-3">
                   <div class="form-group mt-5">
                    <label for="">Isyak</label>
                    <select name="isyak" class="form-control">
                     <option value="Hadir"<?php if ($data["ISYAK"] == 'Hadir' ) echo "selected" ; ?>>Hadir</option>
                     <option value="Izin"<?php if ($data["ISYAK"] == 'Izin' ) echo "selected" ; ?>>Izin</option>
                     <option value="Sakit" <?php if ($data["ISYAK"] == 'Sakit' ) echo "selected" ; ?>>Sakit</option>
                     <option value="Alfa"<?php if ($data["ISYAK"] == 'Alfa' ) echo "selected" ; ?>>Alfa</option>
                   </select>
                 </div>
               </div>
               <div class="col-md-3">
               <div class="form-group mt-5">
                ...
                <input value="<?php echo"$data[TANGGAL]"; ?>"  type="date" class="form-control" name="tanggal">
              </div>                
            </div>
