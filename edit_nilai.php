        ...
        <div id="content-wrapper">
          ...
          <div class="container-fluid">
            ...
            <ol class="breadcrumb">
              ...
              <li class="breadcrumb-item active">Ubah Data Nilai</li>
            </ol>
            <?php
              include 'koneksi.php';
              $id = $_GET['id'];
              $query=mysqli_query($koneksi, "SELECT * FROM nilai join siswa on siswa.no_induk=nilai.no_induk join guru on guru.id_guru=nilai.id_guru join mapel on mapel.kd_mapel=nilai.kd_mapel where id_nilai = '$id'");
              while($data = mysqli_fetch_array($query)){
            ?>
            <form method="POST" action="update_nilai" enctype="multipart/form-data" class="col-md-12">
              <div class="row">
                <div class="col-md-4">                
                  <input type="hidden" name="id" value="<?php echo"$data[ID_NILAI]";?>">
                  <div class="form-group">
                    <label for="Password Siswa">Nama Guru   : </label>
                    <b><?php echo"$data[NAMA_GURU]";?></b>
                  </div>
                   
                  <div class="form-group">
                    <label for="Password Orang Tua">Mata Pelajaran  :</label>
                    <b><?php echo"$data[MAPEL]";?></b>
                  </div>  

                  <div class="form-group">
                    <label for="Password Orang Tua">Nama Santri :</label>
                    <b><?php echo"$data[NAMA_SISWA]";?></b>
                  </div>                         
                 
                   <div class="form-group">
                    <label for="Password Orang Tua">Nilai</label>
                    <input value="<?php echo"$data[NILAI]";?>"  type="text" class="form-control" name="nilai">
                  </div>
                </div>
